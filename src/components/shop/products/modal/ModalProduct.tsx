...
import { setActiveProduct, toogleModalProductActions } from '@/store';
import { createProduct, getCategories, putProduct } from '@/store';
import { MySelect, MyTextArea, MyTextInput, ConfirmButtonProduct } from '@/components';
...
export const ModalProduct = () => {
  ...
  useEffect(() => {
    ...
    setImagenInfo(null);
  }, [activeProduct, isOpenModalProductActions.type]);
  ...
  const validationSchemaAdd = Yup.object({
    title: Yup.string()
      .min(3, 'Debe ser de un mínimo de 3 caracteres')
    ...
    stock: Yup.number()
      ...
      .required('Este campo es obligatorio'),
    category: Yup.string()
      .oneOf(getCategoriesIds(), 'Categoría incorrecta')
      .required('Este campo es obligatorio'),
  });
  ...
  const validationSchemaEdit = Yup.object({
    ...
    description: Yup.string()
      .min(3, 'Debe ser de un mínimo de 3 caracteres')
      .max(70, 'No debe tener más de 50 caracteres'),
    price: Yup.number()
      .typeError('El precio debe ser un número')
      .positive('El precio debe ser positivo')
      .required('Este campo es obligatorio'),
    ...
    >
      ...
      >
        <Typography variant='h4' textAlign={'center'}>
          {isOpenModalProductActions.type === ActionModalProductsEnum.ADD
            ? 'Agregar Producto'
            : 'Editar Producto'}
        </Typography>
        <Formik
            ...
            <Form>
              ...
              >
                <Box
                  ...
                  alignItems={'center'}
                >
                  {imagenInfo !== null && <img src={imagenInfo} alt='imagen' width={'100'} />}
                  ...
                  <Input
                    ...
                    id='file'
                    type='file'
                    onChange={onChangeInpuFile}
                  />
                  {alerta.error === true && <Typography className='error'>{alerta.msg}</Typography>}
                </Box>

                <MyTextInput label='Nombre del producto' name='title' />
                <MyTextInput
                  label={`Precio: ${Number(values.price).toLocaleString('es-CL', {
                    style: 'currency',
                    currency: 'clp',
                  })}`}
                  name='price'
                ...
                <ConfirmButtonProduct handleSubmit={handleSubmit} />
              </Box>
            </Form>
          )}
        </Formik>
