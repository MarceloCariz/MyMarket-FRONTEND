...
import { MyTextInput, ConfirmButton } from '@/components';
import { useAppDispatch, useAppSelector } from '@/hooks';
import { ActionModalCategoryEnum } from '@/enums';
import { CategoryI } from '@/interfaces';
import { postCategory, putCategory, toogleModalCategory } from '@/store';

export const ModalCategory = () => {
  const { isOpenModalCategory, activeCategory } = useAppSelector((state) => state.admin);
  const categoryId = useMemo(() => activeCategory && activeCategory._id, [activeCategory]);
  ...
  const handleSubmitAction = (
    values: CategoryI,
    // resetForm: (nextState?: Partial<FormikState<CategoryI>> | undefined) => void,
  ) => {
    console.log(values);
  ...
  return (
    <ModalContainer
      open={isOpenModalCategory.active}
    ...
    >
      ...
      >
        <Typography variant='h4' textAlign={'center'}>
          {isOpenModalCategory.type === ActionModalCategoryEnum.ADD
            ? 'Agregar categoría'
            : 'Editar categoría'}
        </Typography>
        <Formik
          ...
          onSubmit={(values: CategoryI) => handleSubmitAction(values)}
